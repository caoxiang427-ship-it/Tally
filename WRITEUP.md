...
Tally addresses this gap: unlabelled feedback where categories are not known in advance, at a scale where manual review and conversational AI both become unreliable, yet results must still be structured, countable, and reproducible enough for reporting. Two decisions shaped the design from the outset. Success criteria were pre-registered before any evaluation. And, because open-ended feedback tends to organise by issue rather than by predefined product category, Tally discovers themes from the data rather than assuming a taxonomy, an observation that shaped the evaluation itself.

## Approach

Tally uses a two-pass pipeline. The first pass discovers recurring themes directly from the data by running theme discovery on several independent samples and merging the results, retaining only themes that appear consistently across samples. Classical unsupervised alternatives (embedding-based clustering, or topic models such as LDA) were rejected because they return word clusters that still need manual labelling to become reportable themes, whereas the LLM returns named, human-readable themes directly. An optional domain context (e.g. "a restaurant", "a banking app") can be supplied to focus discovery toward domain-relevant themes. The second pass classifies every comment against the resulting theme list, assigning a primary theme, optional secondary themes, a sentiment label, and a self-reported confidence score. Both passes run at temperature 0 to maximise consistency and reproducibility.

Traditional supervised classifiers, such as TF-IDF-based models, require labelled in-domain training data before they can be used; Tally instead generates a usable theme set directly from raw, unlabelled text. To evaluate classification independently of discovery, a bypass mode accepts a fixed set of reference categories in place of the discovered themes. The resulting per-comment classifications also power higher-level analyses, including multi-label counts, comparisons across time periods, metadata-based segment analysis, and statistical significance testing.
...
At the measured token usage, classification costs approximately $0.38 per 1,000 comments using the production model. A frontier model was evaluated as a higher-cost alternative. Despite costing 3.3× more, it improved Macro-F1 by only 0.006 and Cohen's κ by 0.007, differences well within measurement noise. The lower-cost model is therefore the appropriate production choice; the comparison is itself a useful finding: additional compute did not meaningfully improve accuracy.

Sequential processing requires approximately 18 minutes per 1,000 comments, representing an upper-bound latency. Introducing thread-based parallelism reduced the processing time for 200 comments from around 3 minutes to 37 seconds. Compared with manual coding, which typically requires 4 to 8 hours and costs approximately $100–240 per 1,000 comments, Tally is 2 to 3 orders of magnitude cheaper while completing the analysis in minutes, without sacrificing the accuracy shown above.

The evaluation was intentionally scoped to 6 of the approximately 20 CFPB categories. Performance would likely decline when extended to the full taxonomy, and this has been identified as future work rather than claimed as a current capability.
